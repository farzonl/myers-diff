 ...
 * Copyright 2021 Farzon Lotfi
 * https://github.com/google/diff-match-patch
 * https://github.com/gritzko/myers-diff
 * https://github.com/farzonl/myers-diff
 *
...
class Modifier {
   public:
    static std::string Red() {
        std::stringstream ss;
        ss << "\033[" << Code::FG_RED << "m";
        return ss.str();
    }
    static std::string Green() {
        std::stringstream ss;
        ss << "\033[" << Code::FG_GREEN << "m";
        return ss.str();
    }

    static std::string Default() {
        std::stringstream ss;
        ss << "\033[" << Code::FG_DEFAULT << "m";
        return ss.str();
    }
...
inline std::string op2chr(Operation op) {
    switch (op) {
        case DELETE:
            return Color::Modifier::Red();
        case INSERT:
            return Color::Modifier::Green();
        case EQUAL:
            return Color::Modifier::Default();
        default:
            throw;
    }
}
...
class Diff {
   ...
   public:
    static std::pair<string, string> compute(std::string buffer1,
                                             std::string buffer2) {
        using MyersStringDiff = MyersDiff<string>;
        MyersStringDiff diff{buffer1, buffer2};
        string right;
        string left;
